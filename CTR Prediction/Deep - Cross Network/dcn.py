...
def build_deep_layer(x0,params):
    # build the hidden layers, sized accoring to the 'hidden_units' param.
    net = x0
    for units in params['hidden_units']:
        net = tf.layers.dense(net,unit=units,activation = tf.nn.relu)
    return net
...
def build_cross_layers(x0,params):
    ...
    for i in range(num_layers):
        x = cross_layers(x0,x,'cross_{}'.format(i))
    return x
...
def dcn_model(features,labels,mode,params):
    ...
    return my_head.create_estimator_spec(
        ...
        labels=labels,
        logits=logits,
        train_op = lambda loss:optimizer.minimize(loss,global_step=tf.train.get_global_step())
    )
